...
const NavItem: React.FC<NavItemProps> = ({ label, icon, isActive, onClick }) => (
    <li>
        <button
            onClick={onClick}
            className={`flex items-center p-3 my-1 w-full text-base font-normal rounded-lg transition duration-150 group ${
                isActive
                    ? 'bg-sidebar-active-bg text-white shadow-lg'
                    : 'text-sidebar-text hover:bg-sidebar-hover-bg'
            }`}
        >
            {React.cloneElement(icon, { className: "w-6 h-6 text-sidebar-text"})}
...
const Sidebar: React.FC<SidebarProps> = ({ activePage, setActivePage }) => {
    return (
        <aside className="w-64 flex-shrink-0" aria-label="Sidebar">
            <div className="flex flex-col h-full overflow-y-auto bg-sidebar-bg shadow-lg px-3 py-4">
